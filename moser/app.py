...
PATTERN = re.compile(r"^(mqx|mpx|mqy|mpy)\.html$")

ORDER = ("mqx", "mpx", "mqy", "mpy")
...
st.set_page_config(page_title="Moser mapping bounding set 3D projections", layout="wide")
st.title("Moser mapping bounding set 3D projections")

if not table:
    st.error("No files matching mqx.html, mpx.html, mqy.html, or mpy.html were found.")
    st.stop()
